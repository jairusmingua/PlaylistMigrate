...
export default async (_: NextApiRequest, res: NextApiResponse) => {
    if (_.query[TokenType.queryString] == TokenType.spotify) {
        ...
        const spotify_token_encoded = await new SpotifyOAuth(service.SpotifyCredentials,process.env.SPOTIFY_TOKEN_URI).getToken(auth_code);
        res.redirect(`/?token=${spotify_token_encoded}`);
    } else if (_.query[TokenType.queryString] == TokenType.youtube) {
        const auth_code:string = _.query["code"].toString();
        const youtube_token_encoded = await new YoutubeOAuth(service.YoutubeCredentials).getToken(auth_code);
